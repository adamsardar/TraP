...
B<disorder.cgi> - Display the disordered architecture for a specified SUPERFAMILY protein.
...
my $include_weak_hits = ($supfam_labels)?0:1;
...
=cut
sub sf_key {
	my $result = "<text x=\"0\" y=\"$dy\" font-size=\"12\">Superfamilies:</text>";
	$dy += 5;
	foreach my $sf (sort {$sf_details{$a}{'label'} <=> $sf_details{$b}{'label'}} keys %sf_details) {
		...
		$result .= "<rect x=\"5\" y=\"$dy\" width=\"10\" height=\"10\" fill=\"$colour\" stroke=\"black\" stroke-width=\"0.5\" />";
		$result .= "<text x=\"17\" y=\"$text_dy\" font-size=\"10\">$key</text>";
		$dy+=12;
	}
	return $result;
}
